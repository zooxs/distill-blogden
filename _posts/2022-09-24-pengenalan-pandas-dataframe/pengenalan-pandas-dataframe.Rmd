...
# Pendahuluan
Pandas DataFrame meruapakan salah satu tipe data yang umum digunakan pada Python, terumatama dalam analisis data. Dataframe merupakan tipe data dua dimensi, *size-mutable* (ukurannya dapat berubah), memiliki nilai data yang heterogen (terdiri dari beberapa tipe data seperti `int`, `str` dan `float`). Data ini dapat dibentuk dari *dictionary* yang berisi `key` sebagai nama kolom dan `value` sebagai nilai data pada kolom.
